...
class User < Principal
  ...
  def self.find_for_open_id(access_token, signed_in_resource=nil)
    ...
    user.update_column(:last_sign_in_at, Time.now) if user && !user.new_record? && user.active?
    user
  rescue => text
    raise text
  end
  ...
  def self.find_first_by_auth_conditions(tainted_conditions, opts = {})
    ...
    if tainted_login = tainted_conditions[:login]
      user = find_by_login(tainted_login)
      return user if user && (user.active? || user.registered?)
 ...
 def rss_key
  ...
  def self.find_by_login(login)
    ...
    if login.present?
      ...
      unless user
        ...
        login = devise_parameter_filter.filter(login: login)[:login]
        user = where("LOWER(login) = :value OR LOWER(email) = :value", value: login).first
      end
      user
    end
  end
  ...
  alias_method :check_password?, :valid_password?
end
